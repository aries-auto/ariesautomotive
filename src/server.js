...
server.get('/api/categories/:id', (req, res) => {
	memcached.get(`api:categories:v1:${req.params.id}`, (err, val) => {
		if (!err && val) {
			...
			.then((resp) => {
				...
				zlib.gzip(JSON.stringify(data), (e, result) => {
					...
					memcached.set(`api:categories:v1:${req.params.id}`, result, 8640, () => {
...
server.get('/api/categories', (req, res) => {
	memcached.get('api:v1:categories', (err, val) => {
		if (!err && val) {
			res.send(val);
			return;
		}
		...
		.then((resp) => {
			...
			memcached.set('api:v1:categories', data, 8640, (e) => {
				if (e) {
					res.error(e);
					return;
				}
